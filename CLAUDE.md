...
- `src/app/(app)/` — the authenticated console (assistant chat home `/`, `/inbox`, `/leads`, `/bookings`, `/knowledge`, `/activity`). The route-group layout checks the session server-side and redirects to `/login`.
...
- Fonts: Inter via `next/font` (`--font-sans`), exposed on `<html>` in `layout.tsx`.
